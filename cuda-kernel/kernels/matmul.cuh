...
>
__device__ void compute_fragment(
        const float *a_frag,
        const float *b_frag,
        float *c_frag
) {
...
namespace matmul {
    ...
    __global__ void cache_blocking(
        ...
        for (unsigned int k = 0; k < K; k += BK) {
            shared_a[local_y * (BK + 1) + local_x] = matrix_a[global_a_offset + local_y * K + k + local_x];
            shared_b[local_y * (BN + 1) + local_x] = matrix_b[global_b_offset + (local_y + k) * N + local_x];

            __syncthreads();

            for (unsigned int i = 0; i < BK; i++) {
                sum += shared_a[local_y * (BK + 1) + i] * shared_b[i * (BN + 1) + local_x];
            }
            __syncthreads();
        }
...
//        float tmp[TM][TN] = {0.}; // 每个线程负责TM*TN个元素，则需要申请TM*TN个寄存器保存累加值，额外的一个寄存器用于缓存；
    ...
    template<
            ...
            const unsigned int TN
    >
    __global__ void padded_block_tiling_2d(
            const float alpha,
            const float *A,
            const float *B,
            const float beta,
...
//        float tmp[TM][TN] = {0.}; // 每个线程负责TM*TN个元素，则需要申请TM*TN个寄存器保存累加值，额外的一个寄存器用于缓存；
        ...
        for (int k = 0; k < K; k += BK) {
            for (int i = 0; i < BM; i += a_tile_stride) {
                As[(a_tile_row + i) * (BK + 1) + a_tile_col] = A[a_global_offset + (a_tile_row + i) * K + a_tile_col];
            }
            for (int i = 0; i < BK; i += b_tile_stride) {
                Bs[(b_tile_row + i) * (BN + 1) + b_tile_col] = B[b_global_offset + (b_tile_row + i) * N + b_tile_col];
            }
            __syncthreads();
            a_global_offset += BK;
            b_global_offset += BK * N;
            ...
            for (int kk = 0; kk < BK; kk++) {
                for (int i = 0; i < TM; i++) {
                    for (int j = 0; j < TN; j++)
                        c_tile[i * TN + j] += As[(ty + i) * (BK + 1) + kk] * Bs[tx + j + kk * (BN + 1)];
                }
            }
            __syncthreads();
        }
...
//        float accum[TM][TN] = {0.}; // 每个线程负责TM*TN个元素，则需要申请TM*TN个寄存器保存累加值，额外的一个寄存器用于缓存；
        ...
        for (int m = 0; m < TM; m++) {
            for (int n = 0; n < TN; n += 4) {
                ...
                ctmp.x = alpha * c_frag[m * TN + n] + beta * ctmp.x;
                ctmp.y = alpha * c_frag[m * TN + n + 1] + beta * ctmp.y;
                ctmp.z = alpha * c_frag[m * TN + n + 2] + beta * ctmp.z;
                ctmp.w = alpha * c_frag[m * TN + n + 3] + beta * ctmp.w;
